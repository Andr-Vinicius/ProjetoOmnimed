...
if (isset($_SESSION['usuario']['email'])) {
    ...
    $agendamentos = $agendamentoModel->obterAgendamentosPorIdPaciente($paciente->PAC_ID);
    $isPaciente = true;
} else if (isset($_SESSION['usuario']['prontuario'])) {
    ...
    <header id="header" class="fixed-top">
        <div class="container d-flex align-items-center">
            ...
            <?php } else if (isset($_SESSION['usuario']['prontuario'])) { ?>
                <a href="/medicos/logado" class="appointment-btn scrollto"><span class=" d-md-inline">Perfil</a>
            <?php } ?>
        ...
        <section id="counts" class="counts">
            <div class="container">
                <div class="section-title margem">
                    ...
                    <p>Abaixo, uma listagem dos agendamentos realizados pelos pacientes.</p>
                </div>
                        ...
                        <?php foreach ($agendamentos as $agendamento) { ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    } else if ($agendamento->AGD_STATUS == 2) {
                                ...
                                <td>
                                    <a href="<?php if ($agendamento->AGD_STATUS == 1 && isset($_SESSION["usuario"]["prontuario"])) {
                                                    echo "/agendamentos/confirmar/" . $agendamento->AGD_ID;
                                                } else {
                                                    echo "/agendamentos/detalhar/" . $agendamento->AGD_ID;
                                                } ?>">
                                        <?php if ($agendamento->AGD_STATUS == 1 && isset($_SESSION["usuario"]["prontuario"])) { ?>
                                            Confirmar
                                        <?php } else { ?>
                                            Visualizar
                                        <?php } ?>
                                ...
                                <?php if (!isset($_SESSION["usuario"]["prontuario"])) {
                                    if ($agendamento->AGD_STATUS == 0) { ?>
                                        <td>Agendamento cancelado.</td>
                                    <?php } else if ($agendamento->AGD_STATUS == 1) { ?>
                                        <td>Aguardando confirmação.</td>
                                        <?php } else {
                                        if (($agendamentoModel->triagemSolicitada($agendamento->AGD_ID))) { ?>
                                            <td>Triagem já solicitada</td>
                                        <?php } else { ?>
                                            <td><a href="/triagens/novo/<?php echo $agendamento->AGD_ID; ?>">Solicitar triagem</a></td>
                                        <?php } ?>
            ...
            <div class="row">
                <div class="col-md-3 mt-2">
                    <a href="/" class="appointment-btn scrollto"><span class=" d-md-inline">Voltar</a>
                </div>

                <?php if (sizeof($agendamentos) == 0 && isset($_SESSION["usuario"]["email"])) { ?>
                    <div class="col-md-3 mt-2">
                        <a href="/agendamentos/novo" class="appointment-btn scrollto"><span class=" d-md-inline">Agendar CONSULTA</a>
                    </div>
                <?php } ?>
            </div>
